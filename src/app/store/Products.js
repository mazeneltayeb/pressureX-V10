...
const AddToCartButton = React.memo(function AddToCartButton({ product }) {
  ...
  return (
    <>
      ...
      >
        {productStatus.buttonIcon && <span className="me-1">{productStatus.buttonIcon}</span>}
        {isAuthenticated ? productStatus.buttonText : "سجل الدخول للطلب"}
      </Button>
...
export default function StorePage() {
  ...
  const [sortBy, setSortBy] = useState("newest");
  const [searchNumber, setSearchNumber] = useState("");
  const [availabilityFilter, setAvailabilityFilter] = useState("الكل");
  ...
  // 🔥 2. فلترة وترتيب المنتجات باستخدام useMemo
  const filteredProducts = useMemo(() => {
    if (products.length === 0) return [];
  ...
  // 🔥 4. إعادة تعيين الصفحة عند تغيير الفلاتر
  useEffect(() => {
    setCurrentPage(1);
  }, [searchNumber, selectedCategory, sortBy, availabilityFilter]);
  ...
  return (
    <Container className="py-4 px-md-0">
      ...
      {/* {!authLoading && !isAuthenticated && (
        <Alert variant="warning" className="text-center mb-4">
          <strong>🔒 للاطلاع على الأسعار وإتمام الطلبات</strong>
          <br />
          يرجى <a href="/auth/signin" className="alert-link">تسجيل الدخول</a> أو <a href="/registration" className="alert-link">إنشاء حساب جديد</a>
        </Alert>
      )}
   ...
   <div className="text-center  p-4 bg-light rounded shadow-sm">
        ...
        {!authLoading ? (
          isAuthenticated ? (
            <div>
              <Button variant="success" href="/cart" className="me-2 mb-2 main-button">
                🛒 عرض سلة التسوق
              </Button>
              <Button 
                variant="outline-secondary" 
                href="/trackorders"
                className="mb-2 main-button"
              >
                📦 طلباتي
              </Button>
            </div>
            ...
            <div>
              <Button 
                ...
                variant="success" 
                href="/auth/signin" 
                className="me-2 mb-2 main-button"
              >
                🔓 تسجيل الدخول
              </Button>
              <Button 
